...
class PersistentCheckoutTest
  ...
  def restartCheckoutActor(activeActor: ActorRef, cartActor: ActorRef, system: ActorSystem, id: String): ActorRef = {
    system.stop(activeActor)
    checkoutActorWithResponseOnStateChange(system)(cartActor, id)
  }
  ...
  it should "be in cancelled state after cancel message received in selectingDelivery State" in {
    ...
    expectMsg(selectingDeliveryMsg)
    //restart actor
    val checkoutActorAfterRestart: ActorRef = restartCheckoutActor(checkoutActor, cartActor, system, id)
